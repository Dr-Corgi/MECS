...
def data_to_token_ids_one2many(data_path, target_path, vocabulary_path,
                               tokenizer=None, normalize_digits=True):
    if not gfile.Exists(target_path):
        print("Tokenizing data in %s" % data_path)
        vocab, _ = initialize_vocabulary(vocabulary_path)
...
def prepare_dialog_data_one2many(data_dir, vocabulary_size):
    train_path = get_dialog_train_set_path(data_dir)
    dev_path = get_dialog_dev_set_path(data_dir)
    ...
    train_ids_path = train_path + (".ids%d.in" % vocabulary_size)
    data_to_token_ids_one2many(train_path + ".json", train_ids_path, vocab_path)

    dev_ids_path = dev_path + (".ids%d.in" % vocabulary_size)
    data_to_token_ids_one2many(dev_path + ".json", dev_ids_path, vocab_path)
...
def read_data_one2many(tokenized_dialog_path, max_size=None):
...
if __name__ == "__main__":
    ...
    #prepare_dialog_data_one2many("../data", FLAGS.vocab_size)
    train_data, dev_data, _ = prepare_dialog_data_one2many("../data", FLAGS.vocab_size)
    read_data_one2many(dev_data)